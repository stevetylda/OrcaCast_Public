...
function formatViewingPotentialLabel(
  value: SuggestedPlace["viewingPotential"],
) {
...
function pickLegendColors(colors: string[], colorNoData = false) {
  if (colors.length === 0)
    return ["#08364F", "#0B718D", "#278AA2", "#8EB5BD", "#D7E1DF"];
  const highToLowStops = colorNoData
    ? [1, 0.8, 0.6, 0.4, 0.2, 0]
    : [1, 0.75, 0.5, 0.25, 0];
  const sampled = highToLowStops.map((stop) => {
    const index = Math.max(
      0,
      Math.min(colors.length - 1, Math.round(stop * (colors.length - 1))),
    );
    return colors[index];
  });
...
function isSuggestedPlace(value: unknown): value is SuggestedPlace {
  ...
  return (
    ...
    (candidate.type === "Park" ||
      candidate.type === "Marina" ||
      candidate.type === "Ferry" ||
...
function buildRecommendedPlacesSignature(
...
export function PlannerPage() {
  ...
  const storedDraft = useMemo(
    () => (resumeStoredPlan ? readStoredPlannerDraft() : null),
    [resumeStoredPlan],
  );
  const [plannerSelection, setPlannerSelection] =
    useState<TripPlanSelection | null>(storedSelection);
  const [appliedPlannerSelection, setAppliedPlannerSelection] =
    useState<TripPlanSelection | null>(storedSelection);
  const [plannerOpen, setPlannerOpen] = useState(() =>
    resumeStoredPlan ? readStoredPlannerOpen(!storedSelection) : true,
  );
  const [plannerCollapsing, setPlannerCollapsing] = useState(false);
  const [plannerEditingTransition, setPlannerEditingTransition] =
    useState(false);
  ...
  const [draftCity, setDraftCity] = useState(
    resumeStoredPlan ? (storedDraft?.city ?? "") : "",
  );
  const [draftArrivalDate, setDraftArrivalDate] = useState(
    resumeStoredPlan ? (storedDraft?.arrivalDate ?? "") : "",
  );
  const [draftDepartureDate, setDraftDepartureDate] = useState(
    resumeStoredPlan ? (storedDraft?.departureDate ?? "") : "",
  );
  const [draftMaxTravelDistance, setDraftMaxTravelDistance] = useState(() => {
    ...
    const miles = parsePlannerDistanceInput(
      storedDraft.maxTravelDistance,
      storedUnitsMode,
    );
  ...
  const [pulseSelectedPlaceMarker, setPulseSelectedPlaceMarker] =
    useState(false);
  ...
  const [tripBrushMode, setTripBrushMode] = useState<TripBrushMode | null>(
    null,
  );
    ...
    () =>
      plannerSelection
        ? buildTripPlannerRangeFromDates(
            plannerSelection.arrivalDate,
            plannerSelection.departureDate,
          )
        : null,
    [plannerSelection],
  );
  ...
  const appliedTripRange = useMemo(
    () =>
      appliedPlannerSelection
        ? buildTripPlannerRangeFromDates(
            appliedPlannerSelection.arrivalDate,
            appliedPlannerSelection.departureDate,
          )
        : null,
    [appliedPlannerSelection],
  );
  ...
  const cachedRecommendedPlaces = useMemo(
    () =>
      resumeStoredPlan
        ? readStoredRecommendedPlaces(recommendedPlacesSignature)
        : [],
  ...
  const displayedRecommendedPlaces =
    recommendedPlaces.length > 0
      ...
      : plannerSubmitted && (tripLoading || recommendedPlacesData.isLoading)
        ...
        : recommendedPlaces;
  const recommendedPlacesLoading =
    appliedPlannerSubmitted && (tripLoading || recommendedPlacesData.isLoading);
  const plannerResourcesReady =
    ...
    (appliedPlannerSubmitted &&
      ...
      (renderedTripLoadKey === recommendedPlacesSignature ||
        plannerMapLoadFailed));
  const plannerRevealComplete =
    tripRevealPending && revealMinimumElapsed && plannerResourcesReady;
  ...
  const visiblePlannerMapLocations = useMemo(
    () => [
      ...mapSuggestedPlaces.map(
        (place) => [place.longitude, place.latitude] as [number, number],
      ),
      ...(camerasVisible
        ? cameraLocations.map(
            (camera) => [camera.longitude, camera.latitude] as [number, number],
          )
        : []),
      ...(hydrophonesVisible
        ? hydrophoneLocations.map(
            (hydrophone) =>
  ...
  const handleItineraryDragOver = (
    placeId: string,
    event: ReactDragEvent<HTMLDivElement>,
  ) => {
    if (!draggingItineraryPlaceId || draggingItineraryPlaceId === placeId)
      return;
  ...
  const handleItineraryDrop = (
    targetPlaceId: string,
    event: ReactDragEvent<HTMLDivElement>,
  ) => {
    event.preventDefault();
    const sourcePlaceId =
      draggingItineraryPlaceId ?? event.dataTransfer.getData("text/plain");
  ...
  useEffect(() => {
    ...
    const selectedRow = sidebarRef.current?.querySelector<HTMLElement>(
      `#field-pick-${CSS.escape(selectedPlaceId)}`,
    );
    ...
    () =>
      tripRange
        ? buildHighlightedDays(
            tripRange.startDayOfYear,
            tripRange.endDayOfYear,
            tripRange.crossesYear,
          )
        : new Set<number>(),
    [tripRange],
  );
  ...
  const chartTicks = useMemo<AxisTick[]>(
    () =>
      chartZoomedToDays && tripRange
        ? buildDailyTicks(chartBars, tripRange)
        : buildMonthTicks(weekBars),
    [chartBars, chartZoomedToDays, tripRange, weekBars],
  );
  const weeklyChartTicks = useMemo<AxisTick[]>(
    () => buildMonthTicks(weekBars),
    [weekBars],
  );
  const dailyMonthBands = useMemo<MonthBand[]>(
    () =>
      chartZoomedToDays && tripRange
        ? buildDailyMonthBands(chartBars, tripRange)
        : [],
    [chartBars, chartZoomedToDays, tripRange],
  );
  const chartWindowStyle = useMemo(
    () => buildChartWindowStyle(chartBars),
    [chartBars],
  );
  ...
  const tripWindowSegments = useMemo(
    () => buildInteractiveTripWindowSegments(tripRange, weekBars),
    [tripRange, weekBars],
  );
  const tripLabel = tripRange
    ? formatDisplayDateRange(tripRange.startDate, tripRange.endDate)
    : "Choose dates";
  const tripLengthLabel = tripRange
    ? `${tripRange.dayCount} ${tripRange.dayCount === 1 ? "day" : "days"}`
    : "";
  const tripCityLabel = plannerSelection?.city || "Base location";
  const tripDistanceLabel = formatPlannerDistanceLabel(
    plannerSelection?.maxTravelDistanceMiles,
    unitsMode,
  );
  const activeMaxTravelDistanceMiles =
    appliedPlannerSelection?.maxTravelDistanceMiles ?? null;
  const selectedCount = tripOccurrence?.selectedCount ?? 0;
  const activityLabel = useMemo(
    () => computeActivityLabel(selectedCount, weekBars),
    [selectedCount, weekBars],
  );
  const topWatersLabel = useMemo(
    () => computeTopWaters(displayedRecommendedPlaces),
    [displayedRecommendedPlaces],
  );
  ...
  useEffect(() => {
    ...
    const handlePointerMove = (event: PointerEvent) => {
      ...
      const deltaDays = Math.round(
        (deltaX / Math.max(plot.getBoundingClientRect().width, 1)) *
          TRIP_BRUSH_DAYS,
      );
      const nextSelection = applyTripBrushDelta(
      ...
      setPlannerSelection((current) => {
        ...
        if (
          ...
          current.maxTravelDistanceMiles ===
  ...
  const handleTripBrushPointerDown = (
    mode: TripBrushMode,
  ...
  const handlePlannerSubmit = (event: FormEvent<HTMLFormElement>) => {
    ...
    const range = buildTripPlannerRangeFromDates(
      draftArrivalDate,
      draftDepartureDate,
    );
    if (!range) {
    ...
    const parsedMaxTravelDistanceMiles = parsePlannerDistanceInput(
      draftMaxTravelDistance,
      unitsMode,
    );
    if (tripBrushApplyTimerRef.current) {
    ...
    const nextSelection = {
      ...
      maxTravelDistanceMiles:
        typeof parsedMaxTravelDistanceMiles === "number" &&
        Number.isFinite(parsedMaxTravelDistanceMiles) &&
        parsedMaxTravelDistanceMiles > 0
  ...
  const mapProps = {
    ...
    mapModeLabel: plannerSubmitted
      ? "Loading typical activity map…"
      : "Choose trip details to build your outlook",
    ...
    showTripHotspotMarkers:
      plannerSubmitted && tripHotspotsVisible && !tripLoading,
    selectedPlaceId,
    cameraLocations,
    selectedCameraId,
    selectedHydrophoneId,
    pulseSelectedPlaceMarker: plannerSubmitted
      ...
      <main className="app__main">
        ...
        >
          <div className="plannerResultsPage__main">
              ...
              >
                <header className="plannerResultsPage__promptHeader">
                  ...
                  <div className="plannerResultsPage__promptHeading">
                    <p className="plannerResultsPage__promptEyebrow">
                      Trip planner
                    </p>
                    <h1 id="plannerPromptTitle">
                      {plannerSubmitted
                        ? "Edit Trip Plan"
                ...
                <form
                  className="plannerResultsPage__promptForm"
                ...
                >
                    ...
                    <label className="plannerResultsPage__promptField plannerResultsPage__promptField--distance">
                      ...
                      <span className="plannerResultsPage__promptInputWrap plannerResultsPage__promptInputWrap--suffix">
                        ...
                        <input
                          ...
                          onChange={(event) =>
                            setDraftMaxTravelDistance(event.target.value)
                          }
                          ...
                          autoComplete="off"
                        />
                        <span className="plannerResultsPage__promptSuffix">
                          {unitsMode === "metric" ? "km" : "mi"}
                        </span>
                      </span>
                    </label>
                  </div>
                  ...
                  <div className="plannerResultsPage__promptActions">
                    <button
                      ...
                      disabled={
                        !draftCity || !draftArrivalDate || !draftDepartureDate
                      }
                    >
            ...
            (!isEditingTrip || plannerEditingTransition) ? (
              <section className="plannerResultsPage__summaryCard">
                ...
                <div className="plannerResultsPage__summaryBody">
                  <div className="plannerResultsPage__summaryEyebrow">
                    Your Salish Sea trip
                  </div>
                  <h1>{tripCityLabel}</h1>
                ...
                <div className="plannerResultsPage__summaryActions">
                  <button
                    type="button"
                    className="plannerResultsPage__summaryEditBtn"
                    onClick={openPlannerEditor}
                  >
                    <span
                      className="material-symbols-rounded"
                      aria-hidden="true"
                      ...
                      <button
                        ...
                        onClick={() => {
                          ...
                          primaryMapRef.current?.fitLocations(
                            itineraryPlaces.map(
                              (place) =>
                                [place.longitude, place.latitude] as [
                                  number,
                                  number,
                                ],
                            ),
                        ...
                        <div className="plannerResultsPage__itineraryMenuHead">
                          ...
                          <div className="plannerResultsPage__itineraryMenuActions">
                            <button
                              ...
                              onClick={() =>
                                setItineraryShareMenuOpen((value) => !value)
                              }
                            ...
                            <Fragment key={place.id}>
                              <div
                                className={`plannerResultsPage__itineraryItem${
                                  draggingItineraryPlaceId === place.id
                                    ? " isDragging"
                                    : ""
                                ...
                                onDragOver={(event) =>
                                  handleItineraryDragOver(place.id, event)
                                }
                                onDrop={(event) =>
                                  handleItineraryDrop(place.id, event)
                                }
                                onDragEnd={handleItineraryDragEnd}
                              >
                                <div className="plannerResultsPage__itineraryItemOrder">
                                  {index + 1}
                                </div>
                                <button
                                  type="button"
                                ...
                                <button
                                  ...
                                  onClick={() =>
                                    handleRemovePlaceFromItinerary(place.id)
                                  }
                              ...
                              {index < itineraryPlaces.length - 1 ? (
                                <div
                                  className="plannerResultsPage__itineraryDots"
                                  aria-hidden="true"
                                >
                                  <span />
                                  <span />
                ...
                >
                  ...
                  <div className="plannerResultsPage__itineraryExportCardShell">
                    <div className="plannerResultsPage__itineraryExportCard">
                      ...
                      <div
                        className="plannerResultsPage__itineraryExportRule"
                        aria-hidden="true"
                      />
                      <div className="plannerResultsPage__itineraryExportStops">
                        {itineraryPlaces.map((place, index) => (
                          <div
                            key={`export-${place.id}`}
                            className="plannerResultsPage__itineraryExportStop"
                          >
                            <span>{index + 1}</span>
                            <div>
                      ...
                      <div className="plannerResultsPage__itineraryExportChart">
                        <div className="plannerResultsPage__itineraryExportChartTitle">
                          Typical Sightings Per Week
                        </div>
                        <div
                          className="plannerResultsPage__itineraryExportBars"
                          style={weeklyChartColumnStyle}
                        >
                          {weekBars.map((bar) => (
                            <span
                              ...
                              style={{
                                height: `${Math.max(8, (bar.count / weeklyChartMax) * 100)}%`,
                              }}
                            />
                          ))}
                        </div>
                        <div
                          className="plannerResultsPage__itineraryExportMonths"
                          style={weeklyChartColumnStyle}
                          aria-hidden="true"
                        >
                          {weeklyChartTicks.map((tick) => (
                            <span
                              ...
                              style={{
                                gridColumn: `${tick.index + 1} / span 1`,
                              }}
                            >
                              {tick.label}
                            </span>
            ...
            {showPlannerChrome ? (
              <aside
                className="plannerResultsPage__leftTripStack"
                aria-label="Trip and itinerary"
              >
                <div className="plannerResultsPage__sidebarTripCard">
                  <div
                    className="plannerResultsPage__sidebarTripIcon"
                    aria-hidden="true"
                  >
                  ...
                  <div className="plannerResultsPage__sidebarTripCopy">
                    ...
                    <p>
                      {tripLabel}
                      {tripDistanceLabel ? ` · ${tripDistanceLabel}` : ""}
                    </p>
                  </div>
                  <button
                    type="button"
                    className="plannerResultsPage__sidebarEditTrip"
                    onClick={openPlannerEditor}
                  >
                    Edit trip
                  </button>
                  {itineraryPlaces.length > 0 ? (
                    <button
                      ...
                      onClick={() =>
                        setItineraryExpanded((expanded) => !expanded)
                      }
                ...
                {itineraryPlaces.length > 0 && itineraryExpanded ? (
                  <div
                    id="planner-trip-itinerary"
                    className="plannerResultsPage__leftItineraryCard"
                  >
                    <div className="plannerResultsPage__itineraryPreviewHeader">
                      ...
                      <span className="plannerResultsPage__itineraryHeaderActions">
                        ...
                        {itineraryPlaces.length > 0 ? (
                          <button
                            type="button"
                            className="plannerResultsPage__itineraryExportButton"
                    ...
                    <div
                      className="plannerResultsPage__leftItineraryList"
                      aria-label="Sortable itinerary"
                    >
                      {itineraryPlaces.map((place, index) => (
                        <div
                          key={place.id}
                          className={`plannerResultsPage__leftItineraryItem${draggingItineraryPlaceId === place.id ? " isDragging" : ""}${
                            itineraryDropTargetId === place.id &&
                            draggingItineraryPlaceId !== place.id
                              ? " isDropTarget"
                              : ""
                          }`}
                        ...
                        >
                          ...
                          <button
                            type="button"
                            className="plannerResultsPage__leftItineraryMain"
                            onClick={() => handleSelectItineraryPlace(place)}
                          >
                            <span className="plannerResultsPage__sidebarItineraryOrder">
                              {index + 1}
                            </span>
                            <span>
                              <strong>{place.name}</strong>
            ...
            {showPlannerChrome ? (
              ...
              >
                {spotsCollapsed ? (
                  ...
                  >
                    <span
                      className="plannerResultsPage__spotsNotebookGraphic"
                      aria-hidden="true"
                    >
                    ...
                    <span className="plannerResultsPage__spotsNotebookLabel">
                      Field picks
                    </span>
                    <span className="plannerResultsPage__spotsNotebookCount">
                      {displayedRecommendedPlaces.length}
                    </span>
                  </button>
                ) : (
                  <button
                    ...
                    {sidebarMode === "itinerary" ? (
                      <div className="plannerResultsPage__itineraryPanel">
                        <div className="plannerResultsPage__sidebarSectionHeader">
                          <button
                            type="button"
                            className="plannerResultsPage__sidebarBackBtn"
                            onClick={() => setSidebarMode("overview")}
                          >
                            <span
                              className="material-symbols-rounded"
                              aria-hidden="true"
                          ...
                          <strong>
                            Your itinerary <span>{itineraryPlaces.length}</span>
                          </strong>
                        </div>
                        {itineraryPlaces.length === 0 ? (
                          <div className="plannerResultsPage__itineraryEmptyState">
                            ...
                            <p>
                              Add places from the map to begin building your
                              Salish Sea route.
                            </p>
                          </div>
                        ) : (
                          <div
                            className="plannerResultsPage__sidebarItineraryList"
                            aria-label="Sortable itinerary"
                          >
                            {itineraryPlaces.map((place, index) => (
                              <div
                              ...
                              >
                                ...
                                <button
                                  ...
                                  onClick={() =>
                                    handleSelectItineraryPlace(place)
                                  }
                                >
                                  <span className="plannerResultsPage__sidebarItineraryOrder">
                                    {index + 1}
                      ...
                      <>
                        <div className="plannerResultsPage__itineraryPreviewCard">
                                ...
                                .map((place, index) => (
                                  ...
                                  >
                                    ...
                                    <span>
                                      <b>{place.name}</b>
                                      <small>
                                        {place.viewingPotential === "very-high"
                                          ? "Strong outlook"
                                          : `${formatViewingPotentialLabel(place.viewingPotential)} outlook`}
                                      </small>
                                    </span>
                                  </button>
                                ))}
                          ...
                          <button
                            type="button"
                            className="plannerResultsPage__viewItineraryBtn"
                        ...
                        <div className="plannerResultsPage__spotsHeader plannerResultsPage__sidebarRecommendationsHeader">
                          ...
                          <div>
                            ...
                            <h2>
                              Field Picks{" "}
                              <span>{displayedRecommendedPlaces.length}</span>
                            </h2>
                          </div>
                        </div>

                        <div
                          className="plannerResultsPage__fieldPickFilters"
                          role="group"
                          aria-label="Filter field picks"
                        >
                          {(
                            [
                              ["top", "Top picks"],
                            ...
                            <button
                              ...
                              type="button"
                              className={
                                fieldPickFilter === filter ? "isActive" : ""
                              }
                        ...
                        >
                          {!plannerSubmitted ? (
                            ...
                            <div className="plannerResultsPage__spotsEmptyState">
                              No suggested locations within defined search
                              radius. Expand your search radius and try again.
                            </div>
                          ) : (
                            fieldPickPlaces.map((place, index) => (
                              <div
                                id={`field-pick-${place.id}`}
                                key={place.id}
                                className="plannerResultsPage__spotRow"
                              >
                                <PlannerPlaceCard
                                  place={place}
                                  rank={index + 1}
                                  photoManifest={photoManifest}
                                  itineraryAdded={itineraryPlaceIds.includes(
                                    place.id,
                                  )}
                                  onAddToItinerary={() =>
                    ...
                    {detailPlace ? (
                      <div className="plannerResultsPage__detailModeWrap">
                        <PlannerPlaceDetailView
                          ...
                          onAddToItinerary={() =>
                            handleAddPlaceToItinerary(detailPlace)
                          }
                          onRemoveFromItinerary={() =>
                            handleRemovePlaceFromItinerary(detailPlace.id)
                          }
                          onViewItinerary={() => {
                            setDetailPlaceId(null);
                ...
                >
                  <div className="plannerResultsPage__chartHeader">
                    <div className="plannerResultsPage__chartTitleWrap">
                      ...
                      <div>
                        <h2>
                          {chartZoomedToDays
                            ? "Daily Sightings Around Your Trip"
                            : "Your Trip Window"}
                        </h2>
                        {chartZoomedToDays ? (
                          <p>{tripLabel}</p>
                          ...
                          <p>
                            Enter trip details to highlight your viewing window.
                          </p>
                        )}
                      </div>
                    </div>
                  ...
                  <div className="plannerResultsPage__chartBody">
                    {!plannerSubmitted ? (
                      ...
                      >
                        <div
                          className="plannerResultsPage__chartGridLines"
                          aria-hidden="true"
                        >
                          <span />
                          <span />
                            ...
                            >
                              <div className="plannerResultsPage__tripRangeSelection">
                                <div
                                  ...
                                  onPointerDown={(event) =>
                                    handleTripBrushPointerDown("move", event)
                                  }
                                />
                                <button
                                  type="button"
                                  ...
                                  onPointerDown={(event) =>
                                    handleTripBrushPointerDown("start", event)
                                  }
                                />
                                <button
                                  type="button"
                                  ...
                                  onPointerDown={(event) =>
                                    handleTripBrushPointerDown("end", event)
                                  }
                                />
                              </div>
                            </div>
                        ...
                        <div
                          className="plannerResultsPage__chartBars"
                        ...
                        >
                          {chartBars.map((bar, index) => {
                            return (
                              <div
                                key={bar.index}
                                className="plannerResultsPage__chartBarWrap"
                              >
                                <span
                        ...
                        <div
                          className="plannerResultsPage__chartMonths"
                          aria-hidden="true"
                          style={chartColumnStyle}
                        >
                          {chartTicks.map((tick) => (
                        ...
                        {chartZoomedToDays ? (
                          <div
                            className="plannerResultsPage__chartMonthBands"
                            aria-hidden="true"
                            style={chartColumnStyle}
                          >
            ...
            {showPlannerChrome ? (
              <div className="plannerResultsPage__quickActions">
                {plannerSubmitted ? (
                  <div
                    className={`plannerResultsPage__placesFab${placesMenuOpen ? " isOpen" : ""}`}
                  >
                    <div
                      id="planner-places-filters"
                      className="plannerResultsPage__placesFabMenu"
                      role="group"
                      aria-label="Choose visible points of interest"
                    >
                      {(
                        [
                          ["top", "Top Places", "star"],
                    ...
                    >
                      <span
                        className="material-symbols-rounded plannerResultsPage__placesFabIcon"
                        aria-hidden="true"
                      >
                        add
                      </span>
            ...
            {itineraryExportOpen ? (
              <div
                className="plannerResultsPage__itineraryExportOverlay"
                role="presentation"
                ...
                >
                  <div className="plannerResultsPage__itineraryExportHeader">
                    <div>
                      ...
                      <h2 id="planner-itinerary-export-title">
                        Your itinerary
                      </h2>
                      <p>{tripLabel}</p>
                    </div>
                    <button
                      type="button"
                      className="plannerResultsPage__itineraryExportClose"
                  ...
                  <div className="plannerResultsPage__itineraryExportList">
                    {itineraryPlaces.map((place, index) => (
                      <div
                        ...
                        key={place.id}
                      >
                        <span className="plannerResultsPage__itineraryExportNumber">
                          {index + 1}
                        </span>
                        <div>
                          <strong>{place.name}</strong>
                  ...
                  <div className="plannerResultsPage__itineraryExportFooter">
                    {itineraryPlaces.length}{" "}
                    {itineraryPlaces.length === 1 ? "stop" : "stops"} planned
                  </div>
                </section>
              </div>
            ) : null}
