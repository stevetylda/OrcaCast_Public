...
export function poiTypeMatchesFilters(type: PoiType, filters?: PoiFilters | null) {
...
export function filterPoisByType<T extends { type: PoiType }>(pois: T[], filters?: PoiFilters | null) {
...
function geometryCenter(geometry: Geometry | null | undefined): [number, number] | null {
  ...
  if (geometry.type === "Point") {
    ...
    return Number.isFinite(lon) && Number.isFinite(lat) ? [Number(lon), Number(lat)] : null;
  }
...
function normalizePayload(payload: unknown): PoiDataBundle {
  const items = Array.isArray(payload)
    ...
    : payload && typeof payload === "object" && "items" in payload && Array.isArray((payload as { items?: unknown[] }).items)
      ...
      : payload && typeof payload === "object" && "features" in payload && Array.isArray((payload as { features?: Feature[] }).features)
