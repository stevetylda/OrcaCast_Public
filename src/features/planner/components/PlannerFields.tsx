...
export function PlannerLocationField({ value, options, labelledBy, valueId, onChange }: PlannerLocationFieldProps) {
    ...
    <div className={`plannerResultsPage__locationField${open ? " isOpen" : ""}`} ref={rootRef}>
      <button
        ...
        onClick={() => {
          ...
          else openAt(Math.max(0, options.findIndex((option) => option.name === value)));
        }}
        onKeyDown={(event) => {
          if (event.key === "ArrowDown" || event.key === "ArrowUp") {
            event.preventDefault();
            openAt(Math.max(0, options.findIndex((option) => option.name === value)));
          }
        }}
      ...
      >
        ...
        <span className="material-symbols-rounded plannerResultsPage__locationChevron" aria-hidden="true">expand_more</span>
      </button>

      {open ? (
        <div id={listboxId} className="plannerResultsPage__locationPopover" role="listbox" aria-label="Base location options" onKeyDown={handleListKeyDown}>
          {options.map((location, index) => {
            ...
            return (
              ...
              >
                ...
                {selected ? <span className="material-symbols-rounded" aria-hidden="true">check</span> : null}
              </button>
            );
          })}
        </div>
...
export function PlannerDateRangeField({ arrivalDate, departureDate, labelledBy, valueId, onChange }: PlannerDateRangeFieldProps) {
  const popoverRef = useRef<HTMLDivElement | null>(null);
  const calendarRef = useRef<HTMLDivElement | null>(null);
  const triggerRef = useRef<HTMLButtonElement | null>(null);
  const [open, setOpen] = useState(false);
  const [hoveredDate, setHoveredDate] = useState("");
  const baseVisibleMonth = useMemo(() => startOfUtcMonth(parseIsoDate(arrivalDate) ?? new Date()), [arrivalDate]);
  ...
  useEffect(() => {
    ...
    const onPointerDown = (event: MouseEvent) => {
      ...
      if (!popoverRef.current?.contains(event.target) && !calendarRef.current?.contains(event.target)) setOpen(false);
    };
    const onKeyDown = (event: KeyboardEvent) => {
      if (event.key === "Escape") {
        setOpen(false);
  ...
  const visibleMonth = useMemo(() => addUtcMonths(baseVisibleMonth, visibleMonthOffset), [baseVisibleMonth, visibleMonthOffset]);
  ...
  const focusRelativeDay = (dayIso: string, offset: number) => {
    ...
    calendarRef.current?.querySelector<HTMLButtonElement>(`[data-date="${nextIso}"]`)?.focus();
  };
  ...
  return (
    <div className={`plannerResultsPage__dateRangeField${open ? " isOpen" : ""}`} ref={popoverRef}>
      <button
        ...
        onClick={() => setOpen((value) => { const next = !value; if (next) setVisibleMonthOffset(0); return next; })}
      ...
      {open && typeof document !== "undefined" ? createPortal(
        <>
          <div className="plannerResultsPage__dateRangeBackdrop" aria-hidden="true" onMouseDown={() => setOpen(false)} />
          <div ref={calendarRef} className="plannerResultsPage__dateRangePopover" role="dialog" aria-modal="true" aria-label="Choose trip dates">
          <div className="plannerResultsPage__dateRangePopoverHead">
            ...
            <div className="plannerResultsPage__dateRangeNav">
              <button type="button" onClick={() => setVisibleMonthOffset((current) => current - 1)} aria-label="Previous month"><span className="material-symbols-rounded" aria-hidden="true">chevron_left</span></button>
              <button type="button" onClick={() => setVisibleMonthOffset((current) => current + 1)} aria-label="Next month"><span className="material-symbols-rounded" aria-hidden="true">chevron_right</span></button>
            </div>
          </div>
              ...
              return (
                <section key={month.toISOString()} className="plannerResultsPage__dateRangeCalendar">
                  ...
                  <div className="plannerResultsPage__dateRangeWeekdays">
                    {["S", "M", "T", "W", "T", "F", "S"].map((day, index) => <span key={`${monthLabel}-${day}-${index}`}>{day}</span>)}
                  </div>
                  <div className="plannerResultsPage__dateRangeDays">
                    {buildCalendarCells(month).map((cell) => {
                      ...
                      const isInRange = dayIsWithinSelectedRange(cell.iso, arrivalDate, departureDate) ||
                        (!departureDate && arrivalDate && previewRangeEnd
                          ? dayIsWithinSelectedRange(cell.iso, compareIsoDates(arrivalDate, previewRangeEnd) <= 0 ? arrivalDate : previewRangeEnd, compareIsoDates(arrivalDate, previewRangeEnd) <= 0 ? previewRangeEnd : arrivalDate)
                          : false);
                      return (
                        <button
                          key={cell.iso}
                          ...
                          onKeyDown={(event) => {
                            ...
                            if (offset !== undefined) { event.preventDefault(); focusRelativeDay(cell.iso, offset); }
                          }}
                        ><span>{cell.label}</span></button>
                      );
                    })}
                  </div>
                </section>
          ...
          <div className="plannerResultsPage__dateRangeFooter">
            <span className="plannerResultsPage__dateRangeFooterSummary">{formatPlannerDateRangeSummary(arrivalDate, departureDate)}</span>
            <div className="plannerResultsPage__dateRangeFooterActions">
              <button type="button" className="plannerResultsPage__dateRangeFooterButton isSecondary" onClick={() => { onChange("", ""); setHoveredDate(""); }}>Clear</button>
              <button type="button" className="plannerResultsPage__dateRangeFooterButton isPrimary" onClick={() => { setOpen(false); setHoveredDate(""); window.requestAnimationFrame(() => triggerRef.current?.focus()); }} disabled={!arrivalDate || !departureDate}>Apply dates</button>
            </div>
          </div>
          </div>
        </>,
