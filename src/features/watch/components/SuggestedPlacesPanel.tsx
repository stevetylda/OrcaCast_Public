...
import type { TripLengthOption, TripPlannerOccurrenceResult, TripPlannerRange } from "../../../shared/data/tripPlanner";
...
type SuggestedPlacesPanelProps = {
  ...
  tripCity: string;
  onTripCityChange: (value: string) => void;
  tripLength: TripLengthOption;
  onTripLengthChange: (value: TripLengthOption) => void;
  tripPlannerSearched: boolean;
  tripPlannerLoading: boolean;
  tripPlannerError: string | null;
  tripPlannerRange: TripPlannerRange | null;
  tripOccurrenceSummary: TripPlannerOccurrenceResult | null;
  onTripPlannerSearch: () => void;
  onTripPlannerEdit: () => void;
};
...
export function SuggestedPlacesPanel({
  ...
  const tripDateLabel = tripPlannerRange?.label ?? formatTripDateLabel(tripStartDate);
  ...
  useEffect(() => {
    ...
    const loadPreviews = async () => {
      if (activeView !== "this-week" || places.length === 0) return;
  ...
  }, [activeView, featuredPlace?.id, mapRef, places]);
  ...
  }, [activeView, onLayoutChange, open, places.length, tripPlannerSearched]);
        ...
        <div className="suggestedPlacesPanel__titleGroup">
          ...
          <p className="suggestedPlacesPanel__subtle">
            {activeView === "this-week"
              ...
              : tripPlannerSearched
                ? tripPlannerLoading
                  ? "Loading historical occurrence for your selected travel window."
                  : tripPlannerError
                    ? "Historical occurrence is not available yet for this bundle."
                    : "Seasonal occurrence based on matching historical days-of-year."
                : "Choose dates and an optional starting city."}
          </p>
        </div>
            ...
            <section className="tripPlannerPanel__bookingCard" aria-label="Trip planner search">
              ...
              <label className="tripPlannerField tripPlannerField--wide">
                ...
                <span className="tripPlannerField__control tripPlannerField__control--hero">
                  ...
                  <select value={tripCity} onChange={(event) => onTripCityChange(event.target.value)}>
              ...
              <div className="tripPlannerPanel__bookingGrid">
                <label className="tripPlannerField">
                  ...
                  <span className="tripPlannerField__control tripPlannerField__control--hero">
                    ...
                    <input
                      type="date"
                      value={tripStartDate}
                      onChange={(event) => onTripStartDateChange(event.target.value)}
                    />
                  </span>
                </label>
                ...
                <label className="tripPlannerField">
                  ...
                  <span className="tripPlannerField__control tripPlannerField__control--hero">
                    ...
                    <select
                      value={tripLength}
                      onChange={(event) => onTripLengthChange(event.target.value as TripLengthOption)}
                    >
              ...
              <p className="tripPlannerPanel__footnote">
                Search swaps the forecast for historical occurrence matching those days of year.
              </p>
            </section>
          ) : (
            <>
              ...
              <section className="tripPlannerPanel__results" aria-label="Best viewing opportunities">
                <div className="tripPlannerPanel__resultsHeader">
                  <h3 className="tripPlannerPanel__resultsTitle">Seasonal occurrence loaded</h3>
                  <p className="tripPlannerPanel__resultsSubtle">
                    {tripPlannerLoading
                      ? "The map is building the historical slice."
                      : tripPlannerError
                        ? "Add the trip planner historical data artifact to enable this layer."
                        : tripOccurrenceSummary
                          ? `${Math.round(tripOccurrenceSummary.selectedCount).toLocaleString()} historical sightings across ${tripOccurrenceSummary.activeCells.toLocaleString()} active hexes.`
                          : "No seasonal occurrence has loaded yet."}
                  </p>
                </div>

                {tripPlannerError && (
                  <div className="suggestedPlacesPanel__status suggestedPlacesPanel__status--warning">
                    Missing data file: data/trip_planner/H*_HISTORICAL_DOY.json
                  </div>
                )}
