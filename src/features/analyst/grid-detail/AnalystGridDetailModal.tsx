...
export function AnalystGridDetailModal({
  open,
  onClose,
  darkMode,
  cellId,
  ...
  const activeTab =
    activeTabState.key === detailKey ? activeTabState.tab : "forecast";
  const setActiveTab = (
    tab: "forecast" | "models" | "spread" | "neighbors",
  ) => {
  ...
  const selectedIndex = useMemo(
    () =>
      payload?.selectedSeries.findIndex(
        (point) => point.weekLabel === selectedPeriodKey,
      ) ?? -1,
    ...
    >
      ...
      >
        <div className="modal__header gridDetail__header">
          <div>
            ...
            <div className="modal__subtitle">
              Cell {cellId} · {resolution} ·{" "}
              {payload?.selectedSeries.length ?? periods.length} forecast
              periods
            </div>
          </div>
          <button
            ...
            onClick={onClose}
            aria-label="Close expanded grid view"
          >
        ...
        <div className="modal__body gridDetail__body">
            ...
            payload.selectedSeries.length > 0 && (
              <div className="gridDetail__chartShell">
                ...
                <div className="gridDetail__chartSubtitle">
                  Forecast probability and observed sighting counts across the
          ...
          {activeTab === "models" &&
            ...
            payload.selectedSeries.length > 0 && (
              <div className="gridDetail__chartShell">
                ...
                <div className="gridDetail__chartSubtitle">
                  Week of year vs forecast probability by model
                </div>
                <ModelOverlapChart
                  points={payload.selectedSeries}
          ...
          {activeTab === "spread" &&
            payload?.spreadSeries &&
            payload.spreadSeries.length > 0 && (
              <div className="gridDetail__chartShell">
                <div className="gridDetail__chartTitle">
                  Model Spread And Grid Percentile
                </div>
                <div className="gridDetail__chartSubtitle">
                  Spread band across models with selected-model percentile among
                  all grids
                </div>
                <SpreadPercentileChart
                  points={payload.spreadSeries}
                  selectedIndex={selectedIndex}
                  darkMode={darkMode}
                />
              </div>
          ...
          {activeTab === "neighbors" &&
            payload?.neighborhoodSeries &&
            payload.neighborhoodSeries.length > 0 && (
              <div className="gridDetail__chartShell">
                <div className="gridDetail__chartTitle">
                  Neighborhood Comparison
                </div>
                <div className="gridDetail__chartSubtitle">
                  Center cell and six nearest neighbors across the full forecast
                  window
                </div>
          ...
          {loading && (
            <div className="gridDetail__loading">
              Loading full-period grid history…
            </div>
          )}
          {!loading && error && (
            <div className="gridDetail__loading">{error}</div>
          )}
          {!loading &&
            !error &&
            (!payload || payload.selectedSeries.length === 0) && (
              <div className="gridDetail__loading">
                No time-series data available for this grid.
              </div>
            )}
        </div>
      </section>
...
function GridDetailChart({
  ...
  const yForecast = (value: number) =>
    margin.top + plotHeight * (1 - value / forecastMax);
  const yActual = margin.top;
  const forecastPath = points
    .map(
      (point, index) =>
        `${index === 0 ? "M" : "L"}${xFor(index).toFixed(1)} ${yForecast(point.forecast).toFixed(1)}`,
    )
    .join(" ");
        ...
        >
          ...
          {activeIndex !== null && (
            <>
              <circle
                cx={xFor(activeIndex)}
                cy={yForecast(points[activeIndex].forecast)}
                r={4}
                fill="#19f0d7"
              />
...
function SpreadPercentileChart({
  ...
  const ySpread = (value: number) =>
    margin.top + plotHeight * (1 - value / maxSpread);
  ...
  const selectedPath = points
    .map(
      (point, index) =>
        `${index === 0 ? "M" : "L"}${xFor(index).toFixed(1)} ${ySpread(point.selected).toFixed(1)}`,
    )
    .join(" ");
  const pctPath = points
    .map(
      (point, index) =>
        `${index === 0 ? "M" : "L"}${xFor(index).toFixed(1)} ${yPct(point.percentile).toFixed(1)}`,
    )
    .join(" ");
  ...
  return (
    <div ref={wrapRef} className="gridDetail__chartScroller">
      <svg
        ...
        height={height}
        role="img"
        aria-label="Model spread and percentile chart"
      >
        {leftTicks.map((tick) => (
          <g key={`spread-${tick}`}>
            <line
      ...
      <div className="gridDetail__legend">
        <div className="gridDetail__legendItem">
          <span
            className="gridDetail__legendSwatch"
            style={{ background: "#60a5fa" }}
          />
          IQR and min-max spread
        </div>
        <div className="gridDetail__legendItem">
          <span
            className="gridDetail__legendSwatch"
            style={{ background: "#19f0d7" }}
          />
...
function NeighborhoodComparisonPanel({
  ...
  const displaySeries = useMemo(
    () =>
      series.map((entry) => ({
        ...entry,
        displayForecast: scaledMode
          ? minMaxScale(entry.forecast)
          : entry.forecast,
      })),
    [scaledMode, series],
  );
  const displayForecastMax = scaledMode
    ...
    : Math.max(
        0.01,
        ...displaySeries.flatMap((entry) => entry.displayForecast),
      );
  const yForecast = (value: number) =>
    margin.top + plotHeight * (1 - value / displayForecastMax);
  ...
  const activeSeries =
    displaySeries.find((entry) => entry.cellId === activeCellId) ?? null;
  ...
  return (
    <div ref={panelRef} className="gridDetail__neighbors">
      {activeSeries &&
        ...
        periods[activePointIndex] && (
          <div
            className="gridDetail__tooltip gridDetail__tooltip--neighbors"
            style={{ left: tooltipPos.x, top: tooltipPos.y }}
          >
            ...
            <span>
              Forecast:{" "}
              {formatForecastValue(
                activeSeries.forecast[activePointIndex] ?? 0,
              )}
              {scaledMode
                ? ` · scaled ${formatScaledValue(activeSeries.displayForecast[activePointIndex] ?? 0)}`
                : ""}
            </span>
            <span>
              Actual: {formatCount(activeSeries.actual[activePointIndex] ?? 0)}
            </span>
          </div>
        )}
      <div className="gridDetail__neighborHeader">
        <div className="gridDetail__neighborHeaderCopy">
          <div className="gridDetail__neighborTitle">
            Forecasts with actual-event dots at max
          </div>
      ...
      >
        <svg
          ...
          height={height}
          role="img"
          aria-label="Neighbor comparison chart"
        >
          ...
          {displaySeries.map((entry) => {
            ...
            const forecastPath = entry.displayForecast
              .map(
                (value, pointIndex) =>
                  `${pointIndex === 0 ? "M" : "L"}${xFor(pointIndex).toFixed(1)} ${yForecast(value).toFixed(1)}`,
              )
              .join(" ");
                ...
                {activePointIndex !== null && (
                  <>
                    ...
                    <circle
                      cx={xFor(activePointIndex)}
                      cy={yForecast(
                        entry.displayForecast[activePointIndex] ?? 0,
                      )}
                      r={isActive ? 4.8 : 3.1}
                      fill={color}
                      opacity={opacity}
          ...
          >
            {scaledMode
              ? "Min/Max scaled forecasts with actual-event dots at max"
              : "Forecasts with actual-event dots at max"}
          </text>
          <text
            x={margin.left + plotWidth / 2}
        ...
        {series.map((entry) => {
          ...
          return (
            ...
            >
              <span
                className="gridDetail__legendSwatch"
                style={{ background: getNeighborColor(entry.ringIndex) }}
              />
      ...
      <div
        className={`gridDetail__neighborNote${darkMode ? "" : " gridDetail__neighborNote--light"}`}
      >
        Hover a line or mini-map cell to isolate that neighbor. The center cell
        and six nearest neighbors share the same colors in both the chart and
        the mini-map.
      </div>
    </div>
  );
...
function NeighborMiniMap({
  ...
  useEffect(() => {
    ...
    const onMove = (event: maplibregl.MapMouseEvent) => {
      const feature = map.queryRenderedFeatures(event.point, {
        layers: ["neighbor-focus-fill"],
      })[0];
      const cellId = String(
        (feature?.properties as Record<string, unknown> | undefined)?.cellId ??
          "",
      );
  ...
  return (
    <div
      ref={containerRef}
      className="gridDetail__neighborMiniMap"
      aria-label="Neighbor minimap"
    />
  );
}
...
function buildNeighborhoodMiniMapGeoJson(
  ...
  const contextFeatures: Array<Feature<Polygon | MultiPolygon>> =
    contextPolygons.map((polygon, index) => ({
      type: "Feature",
      properties: {
        kind: "context",
  ...
  const neighborFeatures: Array<Feature<Polygon | MultiPolygon>> = series.map(
    (entry) => ({
      type: "Feature",
      properties: {
        kind: "neighbor",
  ...
  return {
    ...
    features: [...contextFeatures, ...neighborFeatures],
  };
}
...
function buildNeighborhoodContextPolygons(
  ...
  const neighborhoodIds = new Set(
    neighborhoodSeed.map((entry) => entry.cellId),
  );
...
function getSeedBounds(neighborhoodSeed: Array<{ polygons: number[][][][] }>) {
  let minX = Number.POSITIVE_INFINITY;
  let maxX = Number.NEGATIVE_INFINITY;
  let minY = Number.POSITIVE_INFINITY;
  let maxY = Number.NEGATIVE_INFINITY;
...
function computeFeatureCentroid(
  ...
  const sum = coords.reduce<[number, number]>(
    (acc, pair) => [
      acc[0] + Number(pair[0] ?? 0),
      acc[1] + Number(pair[1] ?? 0),
    ],
    [0, 0],
  );
