...
function boundsContains(outer: SurfaceBounds, inner: SurfaceBounds) {
  return (
...
function createSurfaceRasterStyleSignature(
  fc: FeatureCollection,
  width: number,
  height: number,
  paletteColors: string[],
...
function revokeSurfaceRasterUrl(url: string) {
...
function setSurfaceRasterCache(
  fc: FeatureCollection,
  styleSignature: string,
  bounds: SurfaceBounds,
  raster: SurfaceRaster
...
function getSurfaceRasterWorker() {
  if (!supportsSurfaceRasterWorker()) return null;
  if (surfaceRasterWorker) return surfaceRasterWorker;
  try {
    surfaceRasterWorker = new SurfaceRasterWorker();
    return surfaceRasterWorker;
  } catch {
...
function buildSurfaceRasterSync(
  map: MapLibreMap,
  fc: FeatureCollection,
  paletteColors: string[],
  scale?: HeatScale | null
  ...
  const bounds = {
    ...
    maxLng: roundForSignature(targetBounds.maxLng),
    minLat: roundForSignature(targetBounds.minLat),
  ...
  setSurfaceRasterCache(fc, styleSignature, bounds, raster);
  return raster;
}
...
export function addSurfaceOverlay(
  ...
  const surface = buildSurfaceSamples(fc);
  if (!surface) return;
  const targetBounds = resolveSurfaceRenderBounds(map, surface.bounds);
  const bounds = {
    minLng: roundForSignature(targetBounds.minLng),
    maxLng: roundForSignature(targetBounds.maxLng),
    minLat: roundForSignature(targetBounds.minLat),
  ...
  const worker = getSurfaceRasterWorker();
  if (!worker) {
    const raster = buildSurfaceRasterSync(map, fc, paletteColors, scale);
    if (!raster) return;
  ...
  if (
    pendingSurfaceRasterRequest?.fc === fc &&
    pendingSurfaceRasterRequest.styleSignature === styleSignature &&
    pendingSurfaceRasterRequest.bounds.minLng === bounds.minLng &&
    pendingSurfaceRasterRequest.bounds.maxLng === bounds.maxLng &&
    pendingSurfaceRasterRequest.bounds.minLat === bounds.minLat &&
    pendingSurfaceRasterRequest.bounds.maxLat === bounds.maxLat
  ) {
    return;
  }

  const requestId = ++surfaceRasterRequestId;
  latestSurfaceRasterRequestId = requestId;
  pendingSurfaceRasterRequest = { fc, styleSignature, bounds, requestId };

  const handleMessage = (event: MessageEvent<SurfaceRasterWorkerResponse>) => {
    ...
    setSurfaceRasterCache(fc, styleSignature, response.bounds, raster);
    try {
      updateSurfaceSource(map, raster);
    } catch {
      revokeSurfaceRasterUrl(url);
    }
  };
  ...
  const request: SurfaceRasterWorkerRequest = {
    id: requestId,
    fc,
    bounds,
    width,
